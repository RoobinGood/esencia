...
(function (root, factory) {
	...
	var viewOptions = ['models', 'collections', 'data', 'views', 'viewsEvents',
		'urlParams', 'templateHelpers', 'container'];
	...
	View.constructor = function(options) {
		...
		if (!(this.el || options.el || options.container)) {
			this.noel = true;
		}

		// set container to this.el if view belong to controller
		// this.el = options.container ? options.container : this.el;
		...
		// add this.options like old Backbone did for compatibility
		...
		console.log(this.el, this.container);
	};

	/*
	 * Setter for this.container, using for set container of nested views
	 */

	View.setContainer = function(container) {
		this.container = container;
		// console.log('setContainer', arguments, this.el);
	}
	...
	View.setHtml = function(data) {
		...
		if (this.noel && this.isAttached()) {
			this.$el.replaceWith($templateHtml);
		} else {
			if (this.parent) {
				this.parent.$(this.container).html(templateHtml);
			} else {
				$(this.container).html(templateHtml);
			}

			console.log($(this.container).children().first());

			this.setElement($(this.container).children().first(), true);
		}
	...
	View._updateViews = function(method, views, selector, index) {
		...
		switch (method) {
			...
			case 'insert':
				...
				_(views).each(function(view, viewIndex) {
					view.parent = self;

					view.setContainer(selector, index + viewIndex);
				});
			...
			case 'remove':
				...
				_(viewObjs).each(function(viewObj) {
					...
					splice.call(self.views[selector], viewObj.index, 1);
	...
	View.attach = function() {
		...
		this.$('.esencia-view').each(function() {
		...
		// attach current view
		this.$el.data('view', this).addClass('esencia-view');
	...
	View.detach = function() {
		...
		this.$el.removeData('view').removeClass('esencia-view');
	...
	/*
	 * Get view, that attached to closest element with class `.esencia-view`
	 *
